...
public sealed class ElasticJobSearchService : IJobSearchService
{
    ...
    private static JobResponse MapToResponse(JobDocument doc)
        => new(doc.Id, doc.Name, doc.Status.ToString(), doc.Priority.ToString(),
            doc.RetryCount, doc.MaxRetries, doc.ProgressPercentage,
            doc.ErrorMessage, doc.CreatedBy, doc.CreatedAtUtc, doc.UpdatedAtUtc, doc.CompletedAtUtc);
}
